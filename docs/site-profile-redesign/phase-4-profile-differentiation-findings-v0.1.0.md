...
1. **`profile/profile.go`** — `Profile` struct gains `BrandName`, `LogoPath`, `AccentColor`. Nil-safe accessors `GetSlug()`, `GetBrandName()`, `GetLogoPath()`, `GetAccentColor()` — a nil receiver returns the default profile's corresponding field, so any template that receives a nil `*Profile` renders default branding instead of panicking. The secondary profile's `Name` flips from `"transpara.ai"` to `"Transpara"` (Phase 3's byte-identical-HTML contract ends intentionally). `Name` is retained for backward compatibility.
2. **Placeholder SVG logos** at `/static/logo-transpara.svg` and `/static/logo-transpara.svg` — 32×32, single circle, single letter. Colors match the accent hex so the placeholder visually tracks the accent system until real artwork arrives.
...
**Consequence for Phase 5.** If Phase 5 introduces per-profile *nav* (different links for `transpara` vs `transpara-ai`) or per-profile *dashboard copy*, that still goes through `simpleHeader` / `simpleFooter` + the orphan body templates, not through a new shell. Only if a third profile arrives with a structurally different header/footer should the shell-absorption question re-open.
...
1. **Byte-identical within profile** — `/` (no query param) and `/?profile=transpara-ai` (explicit default slug) both resolve through `profile.Chain{QueryParamResolver{}, DefaultResolver{}}.Resolve(r)` to the *same* `*Profile` pointer (`registry["transpara-ai"]`). Same pointer → identical render, by construction. No runtime divergence is possible between those two URLs.
...
- **Auth scoping per profile.** Google OAuth login, the invite flow (`transpara.ai/join/<token>` URLs), and the anonymous/API-key paths are unchanged. Profile doesn't currently influence who can sign in or what session cookies get set.
- **Per-profile domains.** `https://transpara.ai/join/...` and `https://transpara.ai/app/...` URLs were kept hardcoded. When a `transpara` deployment wants its own hostname, the URL builder has to become profile-aware; that also unlocks rewriting the `canonicalHost` redirect in `cmd/site/main.go` to be profile-driven.
- **Real brand assets.** `static/logo-transpara.svg` and `static/logo-transpara.svg` are placeholder geometry. The real brand system lands alongside whatever design artifact the CEO signs off on.
...
- **`HiveStatusPartial` needed a `p` arg the plan didn't anticipate.** A hardcoded brand string (`Autonomous AI agents building transpara.ai — live.`) lived inside `HiveStatusPartial`, which is the HTMX poll target that refreshes the hive dashboard every 5 seconds. Its signature didn't carry `p`. Adding `p *profile.Profile` as the final arg + updating the single Go caller (`graph/handlers.go:4180`) with the standard `FromContext`+`Default` pattern was straightforward, but it meant the "every body copy site threaded through `p`" commit touched one more Go file than the prompt listed.
...
1. Profile drives visible chrome divergence — brand name, logo, title, `og:site_name`, accent color — across all three shells and the four live orphan pages; the default/transpara-ai profile and the new transpara profile now produce different HTML.
