...
- **version:** v0.6.0 (CFADA rounds 1-3 + CFAR rounds 1-2 resolved)
...
### CFAR round 2 (codex) — 1 blocker → resolved

- **CFAR2-1 (open drawer survives a freshness downgrade):** CFAR round 1 gated *offering* unblock hints/commands to `scan.Freshness == FreshnessCurrent`, but `consoleIssueScanFragment` only cleared the out-of-band `#console-intake-drawer` reset when `s.Freshness == FreshnessUnavailable`. Sequence: operator opens the drawer while the projection is current (commands rendered and copyable), then the 10s intake poll later reports `FreshnessStale` or `FreshnessPartial` — the drawer was left untouched, so the now-invalid `gh issue edit` command from before the downgrade stayed open and copyable even though the freshness gate had already revoked it for any new render. Resolved as a class fix over the whole freshness domain: `consoleIssueScanFragment` now emits the drawer OOB reset whenever `s.Freshness != FreshnessCurrent` — an allowlist with a single explicitly-proven preserve branch (verified-current), not a denylist of known-bad states. Stale, partial, unavailable, and any future/unrecognized freshness value all clear the drawer; only a poll that proves the projection is current is permitted to leave it open. The templ doc comment was rewritten to state this rationale directly (drawer content is only valid from a verified-current projection, so any downgrade clears it).
  - Test coverage: `graph/console_intake_test.go`'s `TestConsoleIssueScanFragmentResetsDrawerOnlyWhenUnavailable` was reworked into table-driven `TestConsoleIssueScanFragmentResetsDrawerUnlessCurrent`, covering the full freshness domain (`FreshnessCurrent`, `FreshnessStale`, `FreshnessPartial`, `FreshnessUnavailable`, plus an unrecognized/future `ConsoleFreshness` value) and asserting the OOB drawer reset is present for every non-current state and absent only for current. The board-surface-always-renders assertion from the prior test was preserved.
